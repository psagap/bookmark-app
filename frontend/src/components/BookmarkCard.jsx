...
import { Play, ExternalLink, MoreHorizontal, Pin, Layers, Trash2, RefreshCw, Check } from "lucide-react";
import { cn } from "@/lib/utils";
import GlowingCard from './GlowingCard';
import CustomTweetCard from './CustomTweetCard';
...
const CardMenuInline = ({ onPin, onCreateSide, onDelete, onRefresh, isPinned, isRefreshing, variant = 'default' }) => {
...
const BookmarkCard = ({ bookmark, onDelete, onPin, onCreateSide, onRefresh, selectionMode, isSelected, onToggleSelect, collection }) => {
        ...
        <div className={cn(
          ...
          <div className="p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="w-8 h-8 rounded-full bg-gruvbox-orange/20 flex items-center justify-center text-gruvbox-orange">
            ...
            <p className="text-sm text-gruvbox-fg leading-relaxed whitespace-pre-wrap">
              {notes || title}
            </p>
          </div>
  ...
  // Tweet Card - Use custom component (keep existing)
  if (isTweet) {
    ...
    return (
      <div className="break-inside-avoid mb-5">
        <div className={cn(
          "bookmark-card relative group rounded-xl overflow-hidden transition-all duration-300 hover:shadow-2xl",
          isSelected && "ring-2 ring-gruvbox-yellow ring-offset-2 ring-offset-gruvbox-bg-darkest"
        )}>
          {selectionMode && (
            <SelectionCheckbox isSelected={isSelected} onToggle={onToggleSelect} />
          ...
          {tweetData ? (
            ...
            // Fallback for tweets without data
            <div className="bg-gruvbox-bg-light p-4 rounded-xl">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <XLogo className="w-5 h-5 text-gruvbox-fg" />
                  <span className="text-gruvbox-fg-muted text-sm">Tweet</span>
                </div>
                <a
                  href={url}
                  target="_blank"
              ...
              <p className="text-gruvbox-fg text-sm line-clamp-3">{title}</p>
            </div>
          )}
  ...
  // YouTube Video Card - Style 2 (overlay text on image)
  if (isVideo) {
    ...
    return (
      <div className="break-inside-avoid mb-5">
        <div className={cn(
          "bookmark-card bookmark-card-overlay group relative rounded-xl overflow-hidden min-h-[320px] transition-all duration-300",
          isSelected && "ring-2 ring-gruvbox-yellow ring-offset-2 ring-offset-gruvbox-bg-darkest"
        )}
        style={{
        ...
        >
          ...
          <a
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <div className="w-16 h-16 rounded-full bg-gruvbox-red/90 flex items-center justify-center shadow-2xl transform group-hover:scale-110 transition-transform">
              <Play className="w-7 h-7 text-white fill-white ml-1" />
            </div>
          </a>

          {/* Content - slides up on hover */}
          <div className="card-data absolute bottom-0 left-0 right-0 px-6 py-5 z-10 transform translate-y-[calc(100%-100px)] group-hover:translate-y-0 transition-transform duration-300">
            <div className="card-content">
              {/* Category pill inline with content - clean placement */}
              <div className="flex items-center gap-2 mb-2">
                <span className="category-pill inline-flex items-center gap-1.5 text-[11px] px-2.5 py-1 rounded-full bg-gruvbox-red/20 text-gruvbox-red-light font-medium">
                  <Play className="w-3 h-3 fill-current" />
                  YouTube
                </span>
              </div>
              <h3 className="text-white text-lg font-light leading-snug line-clamp-2 pr-2">
                <a href={url} target="_blank" rel="noopener noreferrer" className="hover:text-gruvbox-yellow-light transition-colors">
              ...
              <p className="card-text text-white/60 text-sm mt-3 line-clamp-2 h-[40px] pr-2">
                {notes || 'Watch this video on YouTube'}
              </p>
            </div>
          </div>
